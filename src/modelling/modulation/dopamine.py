...
class DopamineModulation:
    ...
    def __init__(self, has_dysfunction: str | None, baseline_dopamine: float = 0.5, learning_rate: float = 0.1) -> None:
        ...
        if self.has_dysfunction == "overactive":
            self.recovery_rate = 0.05   # quick to rebuild
            self.decay_rate = 0.005     # dopamine bleeds slowly

        elif self.has_dysfunction == "depleted":
            self.recovery_rate = 0.005   # very slow to rebuild
            self.decay_rate = 0.05       # dopamine bleeds fast

        # Scaling limits
        self.max_rpe_effect = 0.4
        self.max_reward_influence = 0.6
    ...
    def prediction_error(self, expected_reward: float, actual_reward: float, scaling: float) -> float:
        ...
        # 500 is the max reward magnitude expected in the IGT
    ...
    def update_dopamine(self, reward: float, rpe: float, sensitivity: float) -> tuple[float, float]:
        ...
        if self.has_dysfunction == "overactive":
            ...
            delta *= 1.5 if reward > 0 else 0.2
            sensitivity *= 0.5
        ...
        self.dopamine_level -= self.decay_rate * (self.dopamine_level - self.baseline_dopamine)
        ...
        if self.has_dysfunction == "overactive":
            ...
            perceived_reward = reward * self.dopamine_level

        # Punishment sensitivity amplification
        # Some noise is added to reflect variability in negative perception
        if reward < 0:
            perceived_reward *= (1 + 0.7 * sensitivity + np.random.uniform(0, 0.5))
